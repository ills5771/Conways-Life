...
Game of life is a type of cellular automata developed by John Conway in 1970.
The game has 4 rules: 1. Any live cell with fewer than two live neighbors dies, as if caused by under population. 2. Any live cell with two or three live neighbors lives on to the next generation. 3. Any live cell with more than three live neighbors dies, as if by overpopulation. 4. Any dead cell with exactly three live neighbors becomes a live cell, as if by reproduction.
Each cell is dead or alive(on or off) and has 8 neighbors: left,right,up,down,and diagonally in four directions.
To start building the game, a grid is created. A linear-gradient CSS function was used to create our grid, with each cell representing an x and y coordinate. When the grid is created, all the cells are initially set to off(dead). Clicking on each cell manually or selecting a random or preset setting will change the selected cells to on(alive). Once some cells are selected, we can begin playing the game. Clicking the Play button will begin the game. Here, our algorithm, based on the four rules stated above is implemented to determine the state of the neighboring cells with respect to them being alive or dead. The algorithm will determine which cells will live on the the next generation and which ones will die. Options provided to the user include the play/pause button, preset settings of patterns, clearing the grid, randomizing the grid, and changing the speed of the game at each generation. The higher the number entered, the slower the game is played.
